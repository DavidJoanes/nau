...
var gFocusObj = null;var bRichTextEnabled = 0;var gRichTextField = null;var bTimer=null;var objBeforeOrAfterFocusSave = null; var objToBeFocus = null; var gcurSearchRowId = "";var gActiveBtn = null;var gSetFocusByAjax = false; var inside_win3 = false;var called_win3= false;var gCalendarClose = false;callbackOnDelay_win3= function() {
 var waitobj = document.getElementById("WAIT_win3"); var isSaveAction = false; if(typeof document.win3.ICAction != undefined) {
 if(document.win3.ICAction.value == "#ICSave") {
 waitobj = document.getElementById("SAVED_win3"); isSaveAction = true; }
 }
 if(waitobj) {
 var str = waitobj.style.visibility; if(str.indexOf("visible")!=-1) {
 var delayobj = document.getElementById("Delayed_win3"); if(delayobj){
 ...
 delayobj.innerHTML="<span>Processing Please wait</span>"; inside_win3=true; var session_len = sessionStorage.length; for( i=0; i<session_len; i++) {
 var skey = sessionStorage.key(i); if(skey!=null && /globalInside/.test(skey))
 sessionStorage.removeItem(skey); }
 if (bSessionStorage) {
 sessionStorage.setItem("globalInside_win3", inside_win3); sessionStorage.setItem("sourceform","win3"); }
 }
 }
 }
}

callback_end_win3= function(){
 called_win3 = true; var waitobj = document.getElementById("WAIT_win3"); if(waitobj) {
 var str = waitobj.style.visibility; if( str.indexOf("visible")==-1) {
 var delayobj = document.getElementById("Delayed_win3"); delayobj.style.visibility="visible"; delayobj.style.display="block"; var spannode = delayobj.firstElementChild; if(spannode)
 spannode.style.visibility="hidden"; delayobj.innerHTML="Loading Complete"; setTimeout(initialize_win3,100); }
 }
}

initialize_win3 = function(){
 var delayobj = document.getElementById("Delayed_win3"); delayobj.innerHTML=""; delayobj.style.visibility="hidden"; delayobj.style.display="none"; inside_win3=false; called_win3 = false; sessionStorage.removeItem("globalInside_win3"); sessionStorage.removeItem("sourceform");}
...
window.showhide = function (obj, bShow)
 ...
 obj.style.top = scrollY+'px'; obj.style.right = x+'px'; if(obj.id.indexOf("WAIT_win3") == 0)
 {
 disableInteractionDuringProcessing(obj, true); }
 obj.style.visibility = "visible"; obj.style.display = "block"; setTimeout(callbackOnDelay_win3,2000); }
 else
 {
 if(obj.id.indexOf("WAIT_win3") == 0)
 {
 disableInteractionDuringProcessing(obj, false); }
 obj.style.visibility = "hidden"; obj.style.display = "none"; var globalInside_win3 = sessionStorage.getItem("globalInside_win3"); var srcformname = sessionStorage.getItem("sourceform"); if(!called_win3 && srcformname !=null && srcformname.localeCompare("win3")!=0)
 callback_end_win3(); else if(!called_win3 && globalInside_win3!=null && globalInside_win3.indexOf("true")>=0 )
 callback_end_win3(); }
 }
}
...
function setSaveText_win3(txt)
{
var saveobj = document.getElementById("SAVED_win3");if (saveobj)
 {
 if (txt == "") txt = 'Saving...';  document.getElementById("ptStatusText_win3").innerHTML = txt; showhide(saveobj, true); document.getElementById("saveWait_win3").style.display = ""; }
...
function processing_win3(opt,waittime)
{
var waitobj = document.getElementById("WAIT_win3");var saveobj = document.getElementById("SAVED_win3");var saveWaitObj = document.getElementById("saveWait_win3");var waitobjModal = null;var saveobjModal = null;var saveWaitObjModal = null;var oframe = null;if (MTop())
{
 oframe = MTop().document.getElementById(MTop().PTMODFRAME_ + PT_GetTopmostModalDialogIdCount()); if (oframe && oframe.contentWindow)
 {
 try {
 waitobjModal = oframe.contentWindow.document.getElementById("WAIT_win3"); saveobjModal = oframe.contentWindow.document.getElementById("SAVED_win3"); saveWaitObjModal = oframe.contentWindow.document.getElementById("saveWait_win3"); } catch (e) {}
 }
}
...
if (opt == 2)
{
 showhide(waitobj, false); showhide(waitobjModal, false); setSaveText_win3('Saved'); if (saveWaitObj)
 saveWaitObj.style.display = "none"; if (saveWaitObjModal)
 saveWaitObjModal.style.display = "none"; bTimer = setTimeout("processing_win3(0)",waittime);  if (!isFModeLayout() && (typeof top.ptaiAguide=='object') && top.ptaiAguide.ptaiItemId!=null && top.ptaiAguide.ptaiItemId)
 {AgPostMessage();}
}
}
...
var nLastKey_win3 = 0;var oLastKeyEvent = null;var gKeyDownTarget = null;var keys = [];function findNewList(evt, list, keyCode)
{
...
oLastKeyEvent = evt;var target = getEventTarget(evt);gKeyDownTarget = target;if (!isFModeLayout() && !PT_handleTabKeyForModalDialog(evt))
 return false;if (target && target.form && target.form.name != "win3")
 return findHandler("doKeyDown_" + target.form.name, evt);nLastKey_win3 = getKeyCode(evt);if (isFModeLayout() && nLastKey_win3 == 13 && target)
 {
...
if (nLastKey_win3 != "\t".charCodeAt(0))
 {
 if (isGridNav(evt))
 ptGridObj_win3.doArrowNavGrid(evt); if (isTypeAheadEl(evt))
 ptTAObj_win3.KeyUpDown(evt); if (target.nodeName == "A" && typeof ptIframeHdr != "undefined" && ptIframeHdr && ptIframeHdr.isAccessibility() && nLastKey_win3 == 27)
 ...
 if((browserInfoObj2.isSafari || browserInfoObj2.isChrome || (browserInfoObj2.isFF && FFver >= 66) || browserInfoObj2.isEdge) && nLastKey_win3 == 27)
 {
...
if (isCtrlKey(evt))
 return tryFocus(oFirstTab_win3);if (isAltKey(evt) || (isShiftKey(evt) && !isGridNav(evt)))
 return true;if (target == oLastTab_win3)
 {
 if (oFirstTB_win3)
 {
 if (!bTabOverTB_win3)
 ...
 {
 if (!bTabOverPg_win3)
 return tryFocus(oFirstPg_win3); }
 if (bTabOverNonPS_win3 || oFirstTB_win3 != null || oFirstPg_win3 != null)
 return tryFocus(oFirstTab_win3); return true; }

if (target == oLastTB_win3)
 {
 if (oFirstPg_win3)
 {
 if (!bTabOverPg_win3)
 return tryFocus(oFirstPg_win3); }
 if (bTabOverNonPS_win3 || oFirstPg_win3 != null)
 return tryFocus(oFirstTab_win3); return true; }

if (target == oLastPg_win3)
 {
 if (bTabOverNonPS_win3)
 return tryFocus(oFirstTab_win3); return true; }


if (nLastKey_win3 == "\t".charCodeAt(0) && isTypeAheadEl(evt))
 {
 ptTAObj_win3.KeyUpDown(evt); return true; }

}

function getLastKey_win3()
{
var nTemp = nLastKey_win3;nLastKey_win3 = 0;return nTemp;}

function doKeyUp_win3(evt)
{
var target = getEventTarget(evt);keys[evt.keyCode] = false;var key = getKeyCode(evt);var keyChar = String.fromCharCode(key);if (isCtrlKey(evt) && isAltKey(evt) && key == 83 && (typeof (bDoModal_win3) == "undefined" || !bDoModal_win3))
 { 
...
if (keyChar == "\r" && (!(isButton(target) || isRowAct(target)) || !isFModeLayout()) || key == 27)
 ...
 sCancelName = modObj.sCancelName; if (typeof (bDoModal_win3) != "undefined" && bDoModal_win3 && modalID != null )
 {
 ...
 doUpdateParent(document.win3, sCancelName); }
 else if (isFModeLayout() && bInModal && !isLastLocalModal() )
 submitAction_win3(document.win3, sCancelName); else if (isFModeLayout() && key == 27)
 {
...
if (!routeKeyEvent(evt))
 ...
 {
 if (bCtrl || altKey_win3.indexOf(keyChar) < 0)
 {
 ...
 keyChar = String.fromCharCode(key | 0x40); if (ctrlKey_win3.indexOf(keyChar) < 0)
 bNotMine = true; }
 ...
 {
 if (baseKey_win3.indexOf(keyChar) < 0)
 bNotMine = true; }
...
if (bNotMine) {
 ...
 ptTAObj_win3.GiveOptions(evt); return true;}


var code = getModifiers(evt) + keyChar;if (target && target.name)
 document.win3.ICFocus.value = target.name;if (code == "A8")
 if (!doDeleteKey_win3(target))
 return false;if (code == "A\xbf")
 {
 if (window.FindString_win3
 && document.win3.ICFind
 && findScroll(target))
 {
 if (!FindString_win3(document.win3.ICFind))
 return false; }
 else
 return false
 }
 ...
 || key == 13 && isRowAct(target)) 
{
 ...
 doUpdateParent(document.win3, '#ICReturn'); return; }
 ...
 submitAction_win3(document.win3, event.srcElement.id); }
 else { 
 if(key == 32 && !isFModeLayout() && isButton(target)) {
 if(target.id == "prevmonth" || target.id == "nextmonth"|| target.id == "PTClose") return; else {
 var sScript = target.getAttribute('href'); if(sScript.indexOf("javascript:DatePrompt")!=-1) {
 ...
 submitAction_win3(document.win3, evt.target.id); }
}else
{
 if (target && target.form)
 addchg_win3(target); if (code == 'A5'){
 
 if(isFModeLayout())
 submitAction_win3(document.win3, pid, target.id); else
 submitAction_win3(document.win3, "#KEY" + code + operation, target.id); }else if(bAlt){
 if ((key == 49) && ptCommonObj2.isSavePage(target.getElementsByTagName("Form")[0]) )
 setSaveText_win3('Saving...', true, false);  if(altKey_win3.indexOf(keyChar) >= 0 || keyChar == "2" || keyChar == "3" || keyChar == "4"){
 ...
 var url = "javascript:submitAction_win3(document.win3, \'#KEY" + code + "\')"; if (!saveWarning("", null, "", url))
 return false; }
 if( !ptCommonObj2.isSearchPage(target.form) && (code == "A\xbc" || code == "A\xbe" || code == "A\xde") && gActiveBtn && target.name == null)
 {
 document.win3.ICFocus.value = gActiveBtn.name
 submitAction_win3(document.win3, gActiveBtn.name); }
 else
 submitAction_win3(document.win3, "#KEY" + code); }
 }else{
 if (isFModeLayout() && target && target.id)
 document.win3.ICFocus.value = target.id; submitAction_win3(document.win3, "#KEY" + code); }
}
...
{
var obj = document.getElementById('win3divPAGEBAR');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win3divPSPANELTABS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win3divPSPANELTABLINKS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";}
...
{
return keyHandler_win3(keyCode, bIsAlt, bIsCtrl);}

function keyHandler_win3(keyCode, bIsAlt, bIsCtrl)
{
var keyChar = String.fromCharCode(keyCode);var code = "";if (bIsAlt)
{
 code = "A"; if (bIsCtrl || altKey_win3.indexOf(keyChar) < 0)
 return false;}
else if (bIsCtrl)
{
 code = "C"; keyChar = String.fromCharCode(keyCode | 0x40); if (ctrlKey_win3.indexOf(keyChar) < 0)
 return false;}
else
 return false;code += keyChar;if (code == "A8" || code == "A\xbf" || code == "A7")
 return false;if (target && target.form)
 addchg_win3(target);submitAction_win3(document.win3, "#KEY" + code);return true;}

function doDeleteKey_win3(obj)
{
if (!window.DeleteCheck2_win3)
 return false;var scroll = findScroll(obj);if (!scroll)
 return false;var bFocusable = true;if((obj.nodeName == 'BODY' || obj.id.length == 0) && preID_win3!= null ) {
 var temp = obj.ownerDocument.getElementById(preID_win3); if(typeof temp != 'undefined')
...
if (scroll.id.search(/^(.*)(\$scroll[im]?\$)(.*)$/,"$fdelete$") < 0)
 ...
 return DeleteCheck2_win3(buttonid);if(bFocusable) {
...
var row = bFocusable ? RegExp.$1 : RegExp.$3 - 1 ;if (buttonid.search(/^(.*)(\$fdelete\$)(.*)$/) < 0)
 ...
 return DeleteCheck2_win3(buttonid);return false;}

function doInsertKey_win3(obj)
{
if (!window.AddMultiple_win3)
 return false;var scroll = findScroll(obj);if (!scroll)
...
if (bHasInsertBtn)
{
 ...
 return AddMultiple_win3(document.win3.ICAddCount); if (scroll.id.match(/\$scrolli\$/))
 return true;}
return false;}

function findScroll(obj)
{
var temp = null;if(obj.nodeName == 'BODY' && preID_win3 != null ) {
 temp = obj.ownerDocument.getElementById(preID_win3); if(typeof temp != 'undefined')
 obj=temp;}
...
if (window.doKeyPress_win3 == null)
{
window.doKeyPress_win3 = function (evt)
 {
 oLastKeyEvent = evt; var event = evt ? evt : window.evt; var target = getEventTarget(event); if (target && target.form)
 addchg_win3(target); if (isFModeLayout() && target)
 {
 var sKeyupScript = target.getAttribute('onkeyup'); var bDatePrompt = sKeyupScript != null && sKeyupScript.indexOf("DatePrompt") != -1;  if (sKeyupScript != null && !bDatePrompt || target.getAttribute('onkeydown') != null)
 return; }

 if (target && target.form && target.form.name != "win3")
 ...
 if (!isFModeLayout() && MOpopupObj_win3 && typeof(MOpopupObj_win3.bIs_shown) != "undefined" && MOpopupObj_win3.bIs_shown)
 return MOpopupObj_win3.StopAccess(event);  if (typeof (bDoModal_win3) != "undefined" && bDoModal_win3)
 ...
 document.win3.ICFocus.value = target.name;  if (key == 13) {
 
 if (isTypeAheadEl(event) && ptTAObj_win3.IsGrabHighlighted()) {
 if (isFModeLayout())
 typeAheadPositionTopClear()
 else
 ptTAObj_win3.HideTheBox(); return false; }
...
var oFirstTab_win3 = null;var oLastTab_win3 = null;var oFirstTB_win3 = null;var oLastTB_win3 = null;var oFirstPg_win3 = null;var oLastPg_win3 = null;var nFirstTBIndex = 5000; var nFirstPgIndex = 10000;function checkTabIndex(obj)
{
if (obj.tabIndex && obj.tabIndex >= 0)
{
 if (obj.tabIndex < nFirstTBIndex)
 {
 if (oLastTab_win3 == null || obj.tabIndex > oLastTab_win3.tabIndex)
 oLastTab_win3 = obj; if (oFirstTab_win3 == null || obj.tabIndex < oFirstTab_win3.tabIndex)
 oFirstTab_win3 = obj; }
 else if (obj.tabIndex < nFirstPgIndex)
 {
 if (oLastTB_win3 == null || obj.tabIndex > oLastTB_win3.tabIndex)
 oLastTB_win3 = obj; if (oFirstTB_win3 == null || obj.tabIndex < oFirstTB_win3.tabIndex)
 oFirstTB_win3 = obj; }
 else
 {
 if (oLastPg_win3 == null || obj.tabIndex > oLastPg_win3.tabIndex)
 oLastPg_win3 = obj; if (oFirstPg_win3 == null || obj.tabIndex < oFirstPg_win3.tabIndex)
 oFirstPg_win3 = obj; }
}
}

function setEventHandlers_win3(sFirst, sLast, bMac)
{
var focus1, focus2;focus1 = function (evt) {doFocus_win3(this, true, true);};focus2 = function (evt) {doFocus_win3(this, false, true);};fchange = function(evt) {addchg_win3(this); };var i;if (sFirst!="")
 {
...
var frm = document.win3;if (!frm) return;var frmlen = frm.length;objBeforeOrAfterFocusSave = null; var objBefore = null; var bAfter = 0; var b = navigator.userAgent.toLowerCase();var bIE = browserInfoObj2.isIE;var bIE8 = (bIE && b.indexOf("trident/4.0;") != -1) ? 1:0; for (i = 0; i < frmlen; ++i)
 {
 ...
 {
 var sScript = frmi.getAttribute('onfocus'); if (!(sScript && sScript.indexOf("doFocus_win3") != -1))
 {
 if (!isOnClickCancelBubble(frmi) && typeof frmi.onclick == "function")
...
if (window.setKeyEventHandler_win3 == null)
{
 window.setKeyEventHandler_win3 = function ()
 {
 if (typeof(baseKey_win3) != "undefined")
 {
 document.onkeyup = doKeyUp_win3; if (baseKey_win3.indexOf("\r") >= 0 || baseKey_win3.indexOf("\x1b") >= 0)
 document.onkeypress = doKeyPress_win3; }
 document.onkeydown = doKeyDown_win3; }
}
...
function setFocus_win3(fldname, indx, oDoc0)
{
...
if (bRichTextEnabled == 0)
 ...
 oDoc = document; else
 oDoc = oDoc0; var obj = null; if (oDoc.win3)
 {
 obj = oDoc.win3.elements[fldname]; if (!obj)
 obj = oDoc.win3[fldname]; }
...
function findFocusableElement()
{
 ...
 {
 container = document.getElementById(document.win3.name+"divPAGECONTAINER"); if(container)
...
if (obj && typeof obj.focus != 'undefined' && (typeof bPTFind != 'undefined'
 ...
 ptGridObj_win3.doScrollOnFocus(gn, obj); if (isFModeLayout() && isTouchKeyboard() && obj && ptCommonObj2.isEdibleField(obj))
...
function setFocusSecPageMessage_win3()
{
...
function positionWAIT_win3(){
var waitobj = null;var savedobj = null;var objFrame = top.frames['TargetContent'];if (objFrame) {
 waitobj = objFrame.document.getElementById("WAIT_win3"); savedobj = objFrame.document.getElementById("SAVED_win3");}
else {
 waitobj = document.getElementById("WAIT_win3"); savedobj = document.getElementById("SAVED_win3");}
...
{
if (typeof ptGridObj_win3 == "undefined" || !ptGridObj_win3)
 return null;if (typeof gridList_win3 == "undefined" || typeof gridFieldList_win3 == "undefined")
...
for (var i = 0; i < gridList_win3.length; i++)
{
var gn = gridList_win3[i][0];var gc = document.getElementById('divgc'+gn);if (!gc || typeof gc == "undefined") continue;var gfields = gridFieldList_win3[i];for (var j = 0; j < gfields.length; j++)
{
...
{
var gn = this.isGridEl(evt);if (typeof ptGridObj_win3 != "undefined" && ptGridObj_win3 && gn != null)
{
...
function initTypeAheadEl(el) {
 ...
 icAction += "$" + nsuffix; }
 var serverScript = "ptTAObj_win3.oWin.pAction_win3(ptTAObj_win3.oDoc.win3,'" + icAction + "');"; el.obj = oWin.ptTAObj_win3.SetProperties(el, 0, form.ICTypeAheadID, serverScript, true, false, false, true, true, 500);  }
 ...
 return true; else if (typeof PFieldList_win3 != 'undefined' && PFieldList_win3) {
 var fname = removeOccursNum(el.id); for (var i = 0; i < PFieldList_win3.length; i++) {
 if (fname == PFieldList_win3[i][0] || el.id == PFieldList_win3[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 if (ptCommonObj2.isSearchSearchPage(el.form)) {
 if (typeof EFieldList_win3 != 'undefined' && EFieldList_win3) {
 for (var i = 0; i < EFieldList_win3.length; i++) {
 if (fname == EFieldList_win3[i][0] || el.id == EFieldList_win3[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 }
...
function UpdateBreadCrumbs() {
 ...
 bcUpdater.updateBreadCrumbs(document.win3); bcUpdater.removeRemoteData();  backNavigation.setCookie(); if (!isFModeLayout() && (typeof updClassicHistory == 'function')) {
 updClassicHistory(); }
...
function GetDomData(form, szTC, szActionName) {
 ...
 return;  var divHiddenFields = document.querySelector("#" + "win3" + "divPSHIDDENFIELDS")
 ...
 form.target = szTC; form.ICAction.value = szActionName; submitAction_win3(form, szActionName);}
...
function preSubmitWorkCenter_win3(arg1, arg2) {
 ...
 return preSubmitCheckWorkCenter_win3(arg1,arg2,true);}

function preSubmitCheckWorkCenter_win3(arg1,arg2,bPtreplace) {
if(bPtreplace)
{
...
if ( ( parent.ptIframe && parent.ptIframe.isWorkCenterDirty() ) || ( top.ptalPage.isAccessibilityMode && parent.parent.ptIframe && parent.parent.ptIframe.isWorkCenterDirty()) ) {
 ...
 tgtFrameForm.ICChanged.value='-1'; submitAction_win3(arg1, arg2); return; }
...
function preSubmitProcessSpellcheck_win3(form, name) {
...
function preSubmitProcessCKEDITOR_win3(form, name) {
 if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField();}

function preSubmitProcess_win3(form, name) {
 if ((name.indexOf('$delete') > 0 || name.indexOf('$srt') > 0) && typeof (preID_win3) != 'undefined' && preID_win3 != null)
 preID_win3 = null;  if (form == null) {
 form = document.forms['win3']; form.style.display = 'block'; }
 preSubmitProcessSpellcheck_win3(form, name); preSubmitProcessCKEDITOR_win3(form, name);    if (!isFModeLayout()) {
...
function aAction0_win3(form, name, params, bAjax, bPrompt, sAjaxTrfUrl, bWarning, sScriptAfter, nTrfURLType)
{
...
var modObj = top.document.getElementById(top.PTMOD_ + top.modId);if (modObj) {
 ...
 form.ICModelCancel.value = 1; oLastModalWin.document.chgFldArr_win3 = new Array(); } else {
...
if (typeof sessionStorage != "undefined" &&
 ...
 sessionStorage.removeItem("divPopUpParentId"); sessionStorage.removeItem("keyboardKey"); sessionStorage.removeItem("divPopupClosed"); processing_win3(0, 3000); return;} else if (typeof sessionStorage != "undefined") {
...
if (typeof bSearchDialog_win3 != 'undefined' && !bDoModal_win3 && bSearchDialog_win3 && form.ICAction.value.indexOf("\x1b") >=0)
 {
 processing_win3(0, 3000); return; }
...
if (typeof ptGridObj_win3 != 'undefined' && ptGridObj_win3)
 ptGridObj_win3.saveScrollPos();if (isFModeLayout() && !(typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != ''))
 ...
 waitingObjList = loader.GetWaitingObjectList();if (typeof bAjax != "undefined" && !bAjax && (typeof params == "undefined" || params == null || params == "") && (typeof popupObj_win3.isModalWidget == "undefined" || !popupObj_win3.isModalWidget))
 form.submit();else
 loader = new net2.ContentLoader(postUrl_win3, form, name, null, null, null, params, null, bAjax, bPrompt, null, null, null, sAjaxTrfUrl, bWarning, sScriptAfter, nTrfURLType);if (waitingObjList != null)
 loader.WaitingObjQueue = waitingObjList;}

function pAction_win3(form, name, efieldname)
{
if(typeof CKEDITOR != 'undefined')UpdateEditorLinkedField();if ((loader != null) && (loader.GetInProcess())) return;form.ICAction.value = name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();if (typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != '') {
 ...
 if (pm)
 pm.updateTypeaheadMsgEvent(name); var flag = form.ICActionPrompt.value; form.ICActionPrompt.value = 'false'; var ret = aAction_win3(form, name, "", true, 1); form.ICActionPrompt.value = flag; return ret;}
else {
 
 if (pm)
 pm.updatePromptMsgEvent(name); form.ICActionPrompt.value = 'true'; return aAction_win3(form, name, "", true, true);}
}

function mAction_win3(form, name, height, width, title, bModalElement, bSizeOnLoad)
{
 form.ICAction.value = name; form.ICXPos.value = ptCommonObj2.getScrollX(); form.ICYPos.value = ptCommonObj2.getScrollY(); aAction_win3(form, name, null, true, false);}
...
function changeKeyList_win3(obj, form, name, params)
{
 UpdateSearchHistory(0); aAction_win3(form, name, params); var searchObj = document.getElementById('PT_SEARCH'); addClass(searchObj, 'side'); addClose(searchObj); closeSide1FullScreen(); doCloseModal(MTop().modId); if (obj == window)
 obj = getRowElement(form, name); SetSearchStatus(obj, form);}



function updateFromPrompt_win3(obj, form, name, params) {
 aAction_win3(form, name, params);}

function updateFromInPrompt_win3(obj, form, name, params) {
 var res = []; var arr1 = document.querySelectorAll(".ps_prompt-resultsgrid .ps_grid-body .psc_on"); for (var i = 0; i < arr1.length; i++)
 res.push(arr1[i].parentElement.parentElement.parentElement.querySelector(".ps_box-value").firstChild.nodeValue); var objHidden = document.getElementById("PT_WORK_PT_PROMPT_TEMP_IN"); objHidden.value = res.join(","); addchg_win3(objHidden); aAction_win3(form, name, params);}
...
function doModlessOnLoad_win3() {
 if (window.name.length == 0)
 window.name = "win3"; document.win3.target = window.name;}
...
function doModal(url, bPrompt, oParentWin, sCancelName, form, name, sPopupOptions, sResponseHTML) {
 ...
 window.modWin.onLoadExt_win3(); return; }
...
function setWinParent_win3() {
 
}

function setModWinParentPIA() {
 if (setModWinParent())
 document.win3.ICResubmit.value = 0;}

function doCancelMsg() {
 aAction_win3(document.win3, '#ICCancel');}
...
function doCancel(sCancelName) {
 ...
 document.win3.ICAction.value = "None"; else {
 ...
 var fm = win.document.win3; if (!fm) fm = window.document.win3; fm.ICAction.value = sCloseId; document.win3.ICAction.value = "None"; }
 }
 if (!winParent && sCloseId != "#ICCancel") {
 aAction_win3(document.win3, sCloseId); return; }
 try {
 document.win3.target = winParent.name; if (document.win3.ICAction.value != "None") return; if (typeof window.bMFormSubmit != 'undefined')
 winParent.modWinClosed = null; winParent.nResubmit = 0; if (window.document.win3.enctype.indexOf('multipart') != -1 && typeof window.bMFormSubmit != 'undefined' && !window.bMFormSubmit) {
 winParent.document.win3.ICAction.value = "None"; winParent.MTop().ptCommonObj2.hidePopupMask(); closeModal(modalID); }
 
 else if (sCloseId != "#ICCancel")
 aAction_win3(document.win3, sCloseId); else
 {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + modalID); if ((typeof modObj != 'undefined') && modObj && modObj.bRCFModal &&
 (typeof(winParent.aAction_win3) == 'undefined'))
 {
 try
 {
 closeModal(modalID); }
 ...
 winParent.aAction_win3(document.win3, sCloseId); }
...
function onParentUnload_win3() {
 if ((typeof window.modWin != 'undefined') && window.modWin) closeModWin_win3();}

function doModalOnLoad_win3(bStartUp, bFileDetach, bNoParent) {
 ...
 && winParent.document && typeof winParent.document.win3 != 'undefined')
 winParent.processing_win3(0, 3000); resizeModalDialog_pt(modalID); }
 }
 ...
 if (window.document.win3.enctype.indexOf('multipart') != -1)
...
function doUpdateParent(form, name) {
 ...
 return; }
 if (typeof winParent == 'undefined' || winParent == null ) {
 submitAction_win3(document.win3, name); return; }
 if (winParent.bDefer && name.indexOf("#ICCancel") != -1 && bPromptPage_win3) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win3.ICStateNum.value = document.win3.ICStateNum.value; winParent.aAction_win3(winParent.document.win3, '#ICCancel'); var cObj = winParent.document.getElementById("pt_modals"); if(cObj) cObj.style.display = "none"; } else {
 processing_win3(1, 3000); form.target = winParent.name; form.ICAction.value = name; if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField(); winParent.loader = null; if (pm)
 pm.updateParentMsgData(winParent,name);  winParent.aAction_win3(form, name); }
}

function doModalMFormSubmit_win3(form, name) {
 if (name.indexOf("#ICCancel") != -1) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win3.ICStateNum.value = document.win3.ICStateNum.value; winParent.aAction_win3(winParent.document.win3, '#ICCancel'); return; }
 window.bMFormSubmit = true; submitAction_win3(form, name); doModalMFormClose_win3();}

function doModalMFormClose_win3() {
 winParent.bUpLoadProgress = true; winParent.nResubmit = 1; winParent.document.win3.ICStateNum.value = document.win3.ICStateNum.value; winParent.loader = null; winParent.aAction_win3(winParent.document.win3, 'Resubmit'); if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
...
function doUpdateFirstParent(form, name, firstParent) {
 form.target = firstParent.name; form.ICAction.value = name; if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField(); firstParent.loader = null; firstParent.aAction_win3(form, name); if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
...
strCntr+="A";submitAction_win3(document.win3,strCntr);}





var bIsPagelet_win3 = "false", sPageletName_win3 = "";ResetGlyph_win3 = function() {
 if (typeof(sGlyphList_win3) !== "undefined" && sGlyphList_win3 !== "") {
 HideGlyph(sGlyphList_win3); }
}

setPageletInfoInCtxmenu_win3 = function(isPagelet,sPgltName) {
 bIsPagelet_win3 = (isPagelet === "false") ? "false" : "true"; sPageletName_win3 = (typeof(sPgltName) !== "undefined" && sPgltName && sPgltName !== "") ? sPgltName : "";}
...
};var ptTAObj_win3 = new PT_typeahead();function CreateRCMenu4PVG(sPvtGridName,nContext,nLeft,nTop,sActions,sPvgData,BulkActions)
{
...
strAction += "~"+BulkActions;var ch9=String.fromCharCode(9);var elemPVGAct = document.getElementById("ICPVGActMenu");if (elemPVGAct)
 ...
 elemPVGData.value = sPvgData;submitAction_win3(document.win3,strAction);}

InvokePageAppCls = function(sServname,sData)
{
var sFormName = "win3";var form = document.forms[sFormName];if (typeof form.ICAppClsData != 'undefined')
 form.ICAppClsData.value = sData;var sAppServname = "$APPCLS#"+sServname;submitAction_win3(document.win3,sAppServname);}
...
try {
 ...
 form.ICPVGData.value = sPvgData; var resPTxt = ""; var xmlDoc = null; var sLoader = new net2.ContentLoader(postUrl_win3, form, null, "post",function(){},
...
function doFolder(form, pid, fid) {
 ...
 objHidden.value = fid; addchg_win3(objHidden); submitAction_win3(form, pid);}
...
var bEventContinue= true;var tabSelect;var newTab = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI') 
 ...
 {
 document.win3.ICFocus.value = ""; currentTarget.onclick(); }
 break; case 39: 
 case 40: 
...
var bReturn = false;var tabSelect = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI')
 ...
 document.win3.ICFocus.value = currentTarget.id.replace("_R", "_L"); else if (currentTarget.id.indexOf("ICTAB_SCROLL_L") == 0)
 document.win3.ICFocus.value = currentTarget.id.replace("_L", "_R"); else if (currentTarget.id.indexOf("ICTAB_HIDE") == 0)
 document.win3.ICFocus.value = currentTarget.id; currentTarget.onclick(); break; case 36: 
...
function InvokeFieldChange_win3(sFldName)
{
 submitAction_win3(document.win3, sFldName);}
function onRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML)
{


var sFormName = "win3";OpenRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML,sFormName);if (nOpenMode != "4")
 processing_win3(0, 3000);}
...
{
 var changed = null; var objFrame = null; if (document.win3)
 changed = checkFormChanged(document.win3, null); if ((changed == null || !changed) && top.frames) {
...
{
 if (saveButton == null)
 return; var bZoom = isZoom(); setSaveText_win3('Saving...', true, false); bAutoSave = true; if (bZoom) {
 ...
 var icReturn = oWin.document.getElementById(TOOLS_RETURN); if ((icReturn != null) && (typeof oWin.document.win3.ICAutoSave != "undefined" )) {
 oWin.document.win3.ICAutoSave.value = '1'; icReturn.click(); }
 }
 }
 ...
 if (!bZoom) {
 submitAction_win3(document.win3, saveButton); } else {
 
 setTimeout(function(){submitAction_win3(document.win3,saveButton)}, 5000); }
 }
...
{
if (document.win3 && checkFormChanged(document.win3, null))
 submitAction_win3(document.win3,"#ICSave");}
...
function DisplayNUIRCMenu_win3(FieldName,nLeft,nTop,sMenu) {

 
 
 sMenu = sMenu.replace("\\\'", "&apos;"); var mDivObj = document.getElementById('win3' + 'div' + FieldName + '$divpop'); if (!mDivObj)
 ...
 processing_win3(0, 3000); resetMenu(mDivObj); var sClass = mDivObj.getAttribute("class"); sClass = sClass.replace(" psc_hidden", ""); mDivObj.setAttribute("class", sClass); if (typeof(nLeft) != 'undefined' && typeof(nTop) != 'undefined')
 {
...
function DisplayNUIRCMenu(FieldName,sMenu,nLeft,nTop) {
 DisplayNUIRCMenu_win3(FieldName,nLeft,nTop,sMenu);}

InvokeAppClsService_win3 = function(sFldName,sService,sData)
{
var nPos = sFldName.lastIndexOf("$");var szBase = sFldName;if (nPos != -1)
 ...
 szBase = sFldName+"$APPCLS#"+sService;var sFormName = "win3";var form = document.forms[sFormName];if (typeof form.ICAppClsData != 'undefined')
 form.ICAppClsData.value = sData;submitAction_win3(document.win3,szBase);}
