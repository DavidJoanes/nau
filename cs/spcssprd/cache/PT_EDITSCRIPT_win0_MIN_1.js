...
gFocusObj = document.getElementById(name);if (arguments.length == 0) 
 return confirm("Delete current/selected rows from this page?  The delete will occur when the transaction is saved.");else if(arguments.length ==1 && (typeof event != 'undefined') && event.altKey && bAccessibility_win0 && !isFModeLayout())
...
{
 var sScript = obj.getAttribute('onclick'); if (sScript && (sScript.indexOf("submitAction_") != -1) && (getLastKey_win0() != "\t".charCodeAt(0))) { 
...
var val = obj.value;if (val == '') return false;if (val.search(re) != 0)
{
 ...
 PSdeferError_win0(obj, "Number field format error. This field accepts %1 number. %2", fmt); return true;}
return false;}
...
{
 PSdeferError_win0(obj, "The system does not understand the date you entered.   Type the date using format specified in your personalization settings."); return true;}
