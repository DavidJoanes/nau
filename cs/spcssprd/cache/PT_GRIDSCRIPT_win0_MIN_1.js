...
trgelem[4] = "1";if (trgelem[3]==1)
 ...
 {
 if (document.win0.ICFocus && document.win0.ICFocus != "undefined")
 document.win0.ICFocus.value = id; }
...
function hoverLightTR(sRowHoverColor, sRowOrgColor, bReset, sId)
{
 ...
 {
 if (bReset) 
 { x[j].style.backgroundColor=sRowOrgColor;  if(isClassicPlus())
 { 
 x[j].style.outline="none"; }
 }
 else
 {
 if(isClassicPlus())
 { x[j].style.backgroundColor="#F4F7FC"; x[j].style.outline="1px dotted #1c579c"; x[j].style.outlineOffset="-1px"; } 
 else
...
function invokegridactionmenu(event)
{
 ...
 gridActionMenu.style.position = 'absolute'; else
...
function gridmenukeypress(event)
{
 ...
 break; case 27:
 hideGridActionMenu(); currentNode = currentNode.parentNode.parentNode ; if(currentNode && currentNode.firstElementChild)
 {
 currentNode.firstElementChild.focus(); }
