...
mkeybclosercmenu : function(event) {
 if (event.keyCode == 9) 
 ptrc.mclose(); if (event.keyCode == 38) 
 {
 var i = 0; var op = document.getElementById('ptrcDropdown_RelatedContent').children;  while (op[i].style.display == 'none') 
 i++; op[i].focus(); }
},

marrowkeys : function(ev, func) {
var key = (window.event) ? window.event.keyCode: ev.keyCode;var op = ev.currentTarget || ev.srcElement;op=op.parentElement.children;var i=0;var currenttarget = ev.currentTarget || ev.srcElement ;while(op[i]!= currenttarget && op[i]!= null)i++;if(key == 27) {
 ptrc.mclose(); document.getElementById("ptrcdropdownicon_RelatedContent").focus();}

else if(key==13)
{
if(func == "newwin") ptrc.onNewWindow();else if(func == "refresh") ptrc.onRefresh();else if(func == "hide") {
 ptrc.mclose(); ptrc.onHideRC('true'); }
}
...
else if(key == 38) 
{
 ...
 {
 var classNameForCP = frameHtmlElementForCP.className; if( classNameForCP.indexOf('pt_classic_plus_pre858') !== -1 )
 {
 var rootElememntForCP = document.documentElement; ptUtil.addClass(rootElememntForCP, "pt_classic_plus_pre858"); }
 else if( classNameForCP.indexOf('pt_classic_plus') !== -1 )
 {
